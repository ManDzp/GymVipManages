<%@ tag pageEncoding="UTF-8" description="bar" body-content="empty"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib tagdir="/WEB-INF/tags/menubar" prefix="ywbar"%>
<div id="list_toolbar">
	<div class="searchbar-wrapper">
		<div class="searchbar-info">
			<span class="searchbar-icon">&nbsp;</span>
			<span class="searchbar-title">${listTitle}</span>
		</div>

		<!-- 选择框 -->
		<div class="searchbar-info">
			<span class="searchbar-sw">类别：</span>
			<select id="filetypeen" name="filetypeen" class="easyui-combobox"
				data-options="onSelect: searchByType" editable="false"
				panelHeight="auto" style="width: 60px;">
				<option value="">全部</option>
				<c:forEach items="${fileTypes}" var="fileType">
					<option value="${fileType.key}">${fileType.value}</option>
				</c:forEach>
			</select>
		</div>

		<c:choose>
		<c:when test="${status != '-1'}">
		<!-- 全部时不做分管领导查询 -->
		<div class="searchbar-info">
			<span class="searchbar-sw">分管领导：</span>
			<select id="leadername" name="leadername"
				class="easyui-combobox" data-options="onSelect: searchByType"
				editable="false" panelHeight="auto" style="width: 80px;">
				<option value="">全部</option>
				<c:forEach items="${leaders}" var="leader">
					<option value="${leader.key}">${leader.value}</option>
				</c:forEach>
			</select>
		</div>
		</c:when>
		</c:choose>

		<!-- 检索框 -->
		<div class="div-searchbox">
			<ywbar:queryBtn/>
			<ywbar:advQueryBtn/>
		</div>
	</div>

	<div class="toolbar-wrapper">
		<!-- 功能按钮 -->
		<ywbar:menu />
	</div>
</div>
