...
<div id="list_toolbar">
	<div class="searchbar-wrapper">
		...
		<!-- 选择框 -->
		<div class="searchbar-info">
			<span class="sw">字号：</span> 
			<select id="fileKey" name="fileKey" class="easyui-combobox" data-options="onSelect: searchByKey"
				panelHeight="auto" style="width: 100px;">
				<option value="">全部</option>
				<c:forEach items="${fileKeys}" var="fileKey">
					<option value="${fileKey.value}">${fileKey.value}</option>
				</c:forEach>
			</select>
		</div>
